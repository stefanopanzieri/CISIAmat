...
%get_param(cisia.gcs,'SimulationStatus')
...
%% --------------------------------------------
  if strcmp(name_entity,'CISIA_MASTER')
    ...
    disp('MASTER SETUP')
    drawnow('update')
    cisia.minor_sampling_time=block.DialogPrm(2).Data;
    %cisia.id_run=num2str(1000000000*rand(1,1));
...
if strcmp(name_entity,'CISIA_MASTER')
    if strcmp(get_param(gcs,'FastRestart'),'on')
  ...
  if cisia.loadfrommask==0
        ...
        num_inputs=height(data_input_port);
         ...
         % recupera i nomi delle risorse in uscita
          ...
          mask_values=get_param(gcb,'MaskValues');
          %mask_values{4}=all_inputs;   %***** andrebbe scommentata ma con
          %2020b non funziona
          set_param(gcb,'MaskValues',mask_values);

          save('mask_values','mask_values')
...
elseif strcmp(num2str(strfind(block.DialogPrm(1).Data,'LINK')),'1') ||  strcmp(num2str(strfind(block.DialogPrm(1).Data,'SAVE')),'1') || strcmp(num2str(strfind(block.DialogPrm(1).Data,'ROUTE')),'1') || strcmp(num2str(strfind(block.DialogPrm(1).Data,'RFDB')),'1')% LINK or SAVE or ROUTE or RFDB
